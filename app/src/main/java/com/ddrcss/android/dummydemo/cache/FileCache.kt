...
class FileCache(val context: Context, val gson: Gson) {
    ...
    val cacheDirectory = File(context.cacheDir, DIRNAME).also { it.mkdirs() }
    ...
    fun saveUserFullInfo(
        userFullInfo: UserFullInfo,
        handler: Handler,
        onResult: Consumer<Throwable?>
    ) {
    ...
    fun loadUserFullInfo(
        userId: String,
        handler: Handler,
        onSuccess: Consumer<UserFullInfo>,
        onFailure: Consumer<Throwable>
    ) {
        Thread() {
            try {
